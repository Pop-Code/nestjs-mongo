...
import type { ExceptionFactory } from '../module/interfaces'
...
export class EntityManager {
  ...
  protected readonly models: Map<string, Type<any>> = new Map()
  ...
  async registerModel<Model extends EntityInterface>(name: string, model: Type<Model>): Promise<EntityManager> {
  ...
  getModel(id: string): Type<any> | undefined {
    return this.models.get(id)
  }

  getModels(): Map<string, Type<any>> {
    return this.models
  }
  ...
  getCollectionName<Model extends EntityInterface>(nameOrInstance: Model | Type<Model>): string {
  ...
  getCollection<Model extends EntityInterface>(nameOrInstance: Model | Type<Model>, databaseName?: string) {
  ...
  async find<Model extends EntityInterface>(
    ...
    return cursor.map((data) => this.fromPlain(classType, data))
  }
  ...
  async findOne<Model extends EntityInterface>(
    ...
    const ctx = this.getSessionContext()
    const obj = await this.getCollection(classType).findOne<Model>(query, {
      ...(ctx !== undefined ? { session: ctx.session } : {}),
      ...options
    })
    if (obj === null) {
    ...
    return this.fromPlain(classType, obj)
  }
  ...
  protected async deleteCascade<Model extends EntityInterface>(classType: Type<Model>, entity: Model) {
  ...
  watch<Model extends EntityInterface>(classType: Type<Model>, pipes?: Document[], options: ChangeStreamOptions = {}) {
  ...
  async getRelationship<R extends EntityInterface>(
    obj: any,
    property: string,
    options: FindOptions = {}
  ): Promise<R | undefined> {
    ...
    if (isEmpty(relationMetadata)) {
      throw new Error(
        `The property ${property} metadata @Relationship must be set to call getRelationship on ${getObjectName(obj)}`
      )
    }
    ...
    const _id: ObjectId | undefined | null = obj[property]
    if (!(_id instanceof ObjectId)) {
      return
    }

    const filter: Filter<R> = { _id: _id as Condition<R>['_id'] }
  ...
  async getRelationships<R extends EntityInterface = any>(
    ...
    if (isEmpty(relationMetadata) || relationMetadata === undefined) {
      throw new Error(
        `The property ${property} metadata @Relationship must be set to call getRelationships on ${getObjectName(obj)}`
      )
    }
  ...
  async createIndexs<Model extends EntityInterface>(model: Type<Model>) {
