...
> {
  ...
  async get(_id: ObjectId, ...rest: any[]): Promise<Model> {
    const filter = { _id } as MongoFilter<Model>
  ...
  async delete(_id: ObjectId, ...rest: any[]): Promise<void> {
    const filter = { _id } as MongoFilter<Model>
    ...
    try {
      ...
      if (change.operationType !== 'drop') {
        if (change['fullDocument'] !== undefined) {
          change['fullDocument'] = em.fromPlain(classType, change['fullDocument']) as Model
        }
      }
