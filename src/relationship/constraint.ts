...
import type { IsValidRelationshipValidationArguments, WithRelationshipTest } from './decorators'
import type { RelationshipMetadata } from './interfaces'
      ...
      if (relationMetadata?.isArray === true) {
        ...
        for (const [index, _id] of value.entries()) {
          ...
          if (isEmpty(innerR)) {
            errors.push(
              new Error(
                `The property ${args.property} contains an invalid relationship ${_id.toHexString()} at index ${index}`
              )
            )
