...
describe('Relationship', () => {
  describe('Decorator', () => {
    describe('EntityRelationship', () => {
      ...
      it('EntityRelationship should not have child relationship metadata defined by a class that is extending it (EntityRelationshipFoo)', () => {
        const metadata = getRelationshipMetadataList(EntityRelationship).find((m) => m.property === 'extendedFoo')
    ...
    describe('IsValidRelationship', () => {
      describe('EntityRelationship', () => {
        ...
        it('should test an EntityRelationship with an array relationship save and query (children)', async () => {
          ...
          expect(
            relations.filter(
              (r) => r instanceof EntityTest && (r._id.equals(entityTest1._id) || r._id.equals(entityTest2._id))
            )
          ).toHaveLength(2)
        })
  ...
  describe('Cascades', () => {
    ...
    it('should have relationships cascades', () => {
      ...
      const relCascades = getRelationshipCascadesMetadata(RelationshipEntityLevel1Test, RelationshipEntityLevel2Test)
